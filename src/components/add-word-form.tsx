...
function extractPhonetic(entry: Record<string, unknown>): string {
  ...
  const phonetics = entry.phonetics as
    | Array<{ text?: string; audio?: string }>
    | undefined;
...
export function AddWordForm() {
  ...
  const [state, formAction] = useFormState<WordFormState, FormData>(
    addWord,
    {}
  );
    ...
    <div className="fixed inset-0 bg-black/50 dark:bg-black/60 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto dark:glass dark:bg-white/10 dark:border-white/20">
        <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-white/10 sticky top-0 bg-white dark:bg-transparent dark:backdrop-blur-xl z-10 rounded-t-2xl">
          ...
          <button
            onClick={handleClose}
            className="text-gray-400 hover:text-gray-600 dark:text-white/60 dark:hover:text-white"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form action={formAction} className="p-4 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            >
              Từ vựng{" "}
              <span className="text-red-500 dark:text-accent-pink">*</span>
            </label>
            ...
            {lookupStatus === "error" && word.length >= 2 && (
              <p className="text-xs text-amber-600">
                Không tìm thấy, bạn có thể nhập thủ công
              </p>
            )}
          </div>
          ...
          <div className="space-y-1.5">
            <label
              htmlFor="definition"
              className="text-sm font-medium text-gray-700 dark:text-white/80 flex items-center gap-2"
            >
              Nghĩa tiếng Việt
              {lookupStatus === "loading" && (
                <Loader2 className="w-3 h-3 animate-spin text-gray-400 dark:text-white/50" />
              )}
            </label>
            <Textarea
              id="definition"
          ...
          <div className="space-y-1.5">
            ...
            <div
              className={`p-2.5 border rounded-xl text-sm min-h-[42px] ${
